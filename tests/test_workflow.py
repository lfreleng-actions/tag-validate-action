...
class TestValidationWorkflow:
    ...
    def test_check_signature_requirements_signed_required(self):
        ...
        # Signed but unverified should fail
        unverified_sig = SignatureInfo(
            type="gpg",
            verified=False,
            key_id="12345",
        )
        result_unverified = ValidationResult(tag_name="v1.2.3", config=config)
        assert (
            workflow._check_signature_requirements(unverified_sig, result_unverified)
            is False
        )

        # Verified signature should pass
        verified_sig = SignatureInfo(
            type="gpg",
            verified=True,
            key_id="12345",
        )
        result_verified = ValidationResult(tag_name="v1.2.3", config=config)
        assert (
            workflow._check_signature_requirements(verified_sig, result_verified)
            is True
        )
