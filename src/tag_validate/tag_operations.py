...
class TagOperations:
    ...
    async def fetch_tag_info(
        ...
        logger.info(f"Fetching tag info for '{tag_name}' in {repo_path}")
    ...
    async def clone_remote_tag(
        ...
        """
        logger.info(f"Cloning remote tag: {owner}/{repo}@{tag}")
        ...
        try:
            ...
            logger.info(f"Successfully cloned and fetched tag {tag}")
