...
class GitHubKeysClient:
    ...
    async def get_user_gpg_keys(self, username: str) -> list[GPGKeyInfo]:
        ...
        try:
            ...
            self.logger.info(f"Found {len(keys)} GPG keys for user {username}")
            return keys
    ...
    async def get_user_ssh_keys(self, username: str) -> list[SSHKeyInfo]:
        ...
        try:
            ...
            self.logger.info(f"Found {len(keys)} SSH signing keys for user {username}")
            return keys
    ...
    async def verify_gpg_key_registered(
        ...
        try:
            ...
            for key in user_keys:
                # Check both key_id and any subkeys
                key_matches = False
                ...
                # If we found a match, build the verification result
                if key_matches:
            ...
            # Key not found
            self.logger.info(f"GPG key {key_id} not registered to {username}")
            return KeyVerificationResult(
    ...
    async def verify_ssh_key_registered(
        ...
        try:
            ...
            for key in user_keys:
                # Try to match against the key data
                # This is a simple string match - could be enhanced with
                # proper SSH key parsing and fingerprint calculation
            ...
            # Key not found
            self.logger.info(f"SSH key with fingerprint {public_key_fingerprint} not registered to {username}")
            return KeyVerificationResult(
                key_registered=False,
                username=username,
                key_info=None,
